            ...
            {data.adoptions &&
                <div>
                    <div className='d-flex my-3'>
                        <input className='my-3 p-2 flex-grow-1 fs-3 fw-bold border border-dark border-3 rounded' placeholder=" 🔍 Search" onChange={event => setQuery(event.target.value)} />
                        <select onChange={handleChange} placeholder="select" required type="text" name="select" id="select" className="dropdown-toggle btn fw-bold my-3 py-3 px-1 fs-3 border border-dark border-3 rounded" data-toggle="dropdown" style={{backgroundColor: "#f55c7a", color: "#343a40"}}>
                    ...
                    <table className="table table-striped m-2" style={{backgroundColor: '#fff1ab'}}>
                        ...
                        <tbody>
                            {data.adoptions.map(adoption => {
                                ...
                                } else if (adoption[choice].toLowerCase().includes(query.toLowerCase()) || query === '') {
                                    return (
                                        <tr key={adoption.id}>
                                            <td className="fw-bold fs-5">{adoption.dog.name}</td>
                                            <td className="fw-bold fs-5">{adoption.adopter_name}</td>
