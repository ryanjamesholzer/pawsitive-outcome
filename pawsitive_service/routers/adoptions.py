...
def list_adoptions(repo: AdoptionQueries = Depends(), account_data: dict = Depends(authenticator.get_current_account_data)):
    return AdoptionList(adoptions=repo.get_all_adoptions())

@router.get("/api/adoptions/{id}", response_model=AdoptionOut)
def get_one_adoption(id: int, response: Response, repo: AdoptionQueries = Depends(), account_data: dict = Depends(authenticator.get_current_account_data)):
...
def create_adoption(adoption: AdoptionIn, response: Response, repo: AdoptionQueries = Depends(), account_data: dict = Depends(authenticator.get_current_account_data)):
    return repo.add_adoption(adoption)

@router.delete("/api/adoptions/{id}")
def delete_adoption(id: int, repo: AdoptionQueries = Depends(),
